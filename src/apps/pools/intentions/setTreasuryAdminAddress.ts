...
export interface SetTreasuryAdminAddressIntentionData {
  globalConfigId: string;
  newAddress: string;
}

export class SetTreasuryAdminAddressIntention extends BaseIntention<SetTreasuryAdminAddressIntentionData> {
  txType: TransactionType.Other;

  txSubType: TransactionSubType.SET_TREASURY_ADMIN_ADDRESS;

  constructor(public readonly data: SetTreasuryAdminAddressIntentionData) {
    super(data);
  }
  ...
  async build(input: { client: IotaClient; account: WalletAccount }): Promise<Transaction> {
    ...
    transaction.moveCall({
      ...
      function: config.setTreasuryAdminAddressMethod,
  ...
  static fromData(data: SetTreasuryAdminAddressIntentionData) {
    return new SetTreasuryAdminAddressIntention(data);
  }
}
